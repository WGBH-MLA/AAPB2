...
describe Organization do
 ...
 "      University of Georgia"] }
  it 'contains expected data' do
    ...
    expect(org.facet_url).to eq("/catalog?sort=title+asc&f[contributing_organizations][]=WGBH+%28MA%29")
  end
  ...
  describe '.facet_url' do
    it 'returns a url to all records if there are no digitized records' do
      ...
      expect(wgbh.facet_url).to eq("/catalog?sort=title+asc&f[contributing_organizations][]=WGBH+%28MA%29&f[access_types][]=all")
    end
  end
end
