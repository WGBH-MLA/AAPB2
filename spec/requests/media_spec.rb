...
describe 'Requests to /media/ endpoints', not_on_ci: true, type: :request do
  ...
  before(:all) do
    ...
    record = ingester.ingest_xml_no_commit(pbcore_xml_with_sony_ci_id(@aapb_id, @sony_ci_asset_id))
    ingester.commit
  ...
  after(:all) do
    ...
    begin
      ...
      result = sony_ci.delete("/assets/#{@sony_ci_asset_id}")
    rescue StandardError => e
  ...
  UNAUTHORIZED_REFERRERS = %w(https://www.youtube.com)
