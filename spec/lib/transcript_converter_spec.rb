...
describe TranscriptConverter do
  ...
  describe '.json_to_html' do
    it 'returns formatted HTML from JSON file' do
      expect(Nokogiri::HTML(TranscriptConverter.get_json_parts(json).to_html).errors.empty?).to be true
    end

    it 'returns the expected html' do
      expect(TranscriptConverter.get_json_parts(json).to_html).to eq(json_html)
    end
  end

  describe '.text_to_html' do
    it 'returns formatted HTML from text file' do
      expect(Nokogiri::HTML(TranscriptConverter.get_text_parts(text).to_html).errors.empty?).to be(true)
    end
  end
end
