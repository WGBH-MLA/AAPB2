    ...
    describe 'search constraints' do
      ...
      describe 'facet ORs' do
        ...
        it 'works in the UI' do
          ...
          click_link('Iowa PBS (IA)')
          # TODO: check count when IP set in request.
          expect(page).to have_text('Contributing Organizations: WGBH (MA) OR Iowa PBS (IA)'), missing_page_text_custom_error('Contributing Organizations: WGBH (MA) OR Iowa PBS (IA)', page.current_path)

          expect(page).to have_css('a', text: 'District of Columbia')
          click_link('District of Columbia')
          expect(page).to have_text('WGBH (MA) OR Iowa PBS (IA) OR Library of Congress (DC) OR NewsHour Productions (DC)'), missing_page_text_custom_error('WGBH (MA) OR Iowa PBS (IA) OR Library of Congress (DC) OR NewsHour Productions (DC)', page.current_path)
