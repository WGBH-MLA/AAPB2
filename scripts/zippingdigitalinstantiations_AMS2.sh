...
echo "DigitalInstantiation.filename,Asset.id,DigitalInstantiation.generations,DigitalInstantiation.holding_organization,DigitalInstantiation.aapb_preservation_lto,DigitalInstantiation.aapb_preservation_disk,DigitalInstantiation.md5" > $zipdir/manifest.csv
for f in $(find $ingest -type f); do
	...
	then
		assetid=$( grep ${filename} $ref | cut -d , -f 1 )
		echo $assetid
