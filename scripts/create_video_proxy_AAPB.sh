...
do 
	...
	if mediainfo "$i" | grep '16:9'; then
		ffmpeg -i "$i" -vcodec libx264 -pix_fmt yuv420p -b:v 711k -s 640:360 -acodec aac -ac 2 -b:a 128k "$outDir/$newDir/$(basename $i)".mp4;
	else 
		ffmpeg -i "$i" -vcodec libx264 -pix_fmt yuv420p -b:v 711k -s 480:360 -acodec aac -ac 2 -b:a 128k "$outDir/$newDir/$(basename $i)".mp4;
	fi
done ;
IFS=$OLDIFS
